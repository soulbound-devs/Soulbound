 ...
 * {@link net.minecraft.network.protocol.game.ServerboundContainerClickPacket}.
 * @author rubensworks
 *
 */
	...
	public void actionServer(Level world, ServerPlayer player) {
		...
		if (player.containerMenu.containerId == windowId) {
			if (player.isSpectator()) {
				...
				((BackpackMenu) player.containerMenu).updateCraftingInventory(player, arraylist);
			} else {
				...
				if (flag) {
					...
					((BackpackMenu) player.containerMenu).updateCraftingInventory(player, nonnulllist1);
				} else {
